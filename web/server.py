...
import config
from src.inference.inference_lstm import load_lstm_model, load_scaler, predict_realtime_lstm
...
def get_stats():
    ...
    """
    try:
        if os.path.exists('models/metrics/lstm_metrics.txt'):
            with open('models/metrics/lstm_metrics.txt', 'r') as f:
                lines = f.readlines()
                stats = {}
                for line in lines:
